...
def velocity_plot(gene,u_s,color_scatter,point_size,alpha_inside,colormap,v_min,v_max,save_path,step_i,step_j,show_arrow=True,custom_map=None):
    print(gene)
    plt.figure(None,(6,6))
    ...
    if colormap is not None:
        layer1=plt.scatter(u_s[:, 0], u_s[:, 1],
               alpha=alpha_inside, s=point_size, edgecolor="none",c=pd.factorize(cluster_info)[0], cmap=colormap)
        plt.colorbar(layer1)
    elif custom_map is not None:
        cmap1 = LinearSegmentedColormap.from_list("mycmap", custom_map)
        color_map1=cmap1
        layer1=plt.scatter(u_s[:, 0], u_s[:, 1],
    ...
    if show_arrow:
        plt.scatter(u_s_downsample[:, 0], u_s_downsample[:, 1], # sampled circle
                    color="none",s=point_size, edgecolor="k")
        pcm1 = plt.quiver(
        u_s_downsample[:, 0], u_s_downsample[:, 1], u_s_downsample[:, 2], u_s_downsample[:, 3],
...
def pipline_run_sim_data(load_raw_data,name,ratio):
    ...
    s0_mat.to_csv('/Users/shengyuli/Library/CloudStorage/OneDrive-HoustonMethodist/work/Velocity/data/simulation/data/multi_path/multi_path/build_scv_compatible_raw/s0_'+ratio+'.csv')
    u0_mat.to_csv('/Users/shengyuli/Library/CloudStorage/OneDrive-HoustonMethodist/work/Velocity/data/simulation/data/multi_path/multi_path/build_scv_compatible_raw/u0_'+ratio+'.csv')

    s0_mat=pd.read_csv('/Users/shengyuli/Library/CloudStorage/OneDrive-HoustonMethodist/work/Velocity/data/simulation/data/multi_path/multi_path/build_scv_compatible_raw/s0_'+ratio+'.csv')
    u0_mat=pd.read_csv('/Users/shengyuli/Library/CloudStorage/OneDrive-HoustonMethodist/work/Velocity/data/simulation/data/multi_path/multi_path/build_scv_compatible_raw/u0_'+ratio+'.csv')

    adata_building=sc.read_csv('/Users/shengyuli/Library/CloudStorage/OneDrive-HoustonMethodist/work/Velocity/data/simulation/data/multi_path/multi_path/build_scv_compatible_raw/s0_'+ratio+'.csv', delimiter=',', first_column_names=True, dtype='float32')
    ...
    # dynamical_velocity
    scv.tl.recover_dynamics(adata_building_test, n_jobs=8, n_top_genes = 1000)
    ...
    for gene in list(gene_choice)[0:10]:
    ...
    adata_building_test.write(filename='/Users/shengyuli/Library/CloudStorage/OneDrive-HoustonMethodist/work/Velocity/data/simulation/data/multi_path/multi_path/build_scv_compatible_raw/multipath_'+ratio+'.h5ad')

    # gene velocity plot
    gene_choice=load_raw_data['gene_list'].drop_duplicates()
    # gene_choice=['simulation0','simulation1']

    outpath='/Users/shengyuli/Library/CloudStorage/OneDrive-HoustonMethodist/work/Velocity/data/simulation/data/multi_path/multi_path/velocity_result/scv'
    ...
    for nth,gene in enumerate(gene_choice):
        ...
        # velocity_plot(gene,s0_u0_s1_u1)
        ...
        # velocity_plot(gene,s0_u0_s1_u1_steady)
    ...
    gene_s0_u0_s1_u1_dynamic_and_steady_df.to_csv(os.path.join(outpath,'scvelo_result_multi_path__s0_u0_s1_u1_dynamic_and_steady_df_'+ratio+'.csv'),index=False)
...
if __name__ == "__main__":
    ...
    for ratio in ratio_list:
        print(ratio)
        ratio=str(ratio)
        load_raw_data=pd.read_csv('/Users/shengyuli/Library/CloudStorage/OneDrive-HoustonMethodist/work/Velocity/data/simulation/data/multi_path/multi_path/multi_path_Path1Upper_(1000)__R'+ratio+'.csv')
