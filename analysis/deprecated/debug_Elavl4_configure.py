...
detail = pd.read_csv('/Users/shengyuli/OneDrive - Houston Methodist/work/Velocity/data/velocyto/neuro/test_story/velocyto_in_storyboardLr0.001Costv1C1r0.5C2cf0.3Downneighbors0_200_200Ratio0.125N30OAdam/detail_e200.csv')
details_one_gene = detail[detail.gene_name == gene_choice[0]]
print(details_one_gene)

plt.scatter(pd.to_numeric(details_one_gene.s0), pd.to_numeric(details_one_gene.u0.to_list()))
plt.quiver(pd.to_numeric(details_one_gene.loc[:].s0), pd.to_numeric(details_one_gene.loc[:].u0),
pd.to_numeric(details_one_gene.loc[:].s1)-pd.to_numeric(details_one_gene.loc[:].s0), pd.to_numeric(details_one_gene.loc[:].u1)-pd.to_numeric(details_one_gene.loc[:].u0)
)


plt.quiver(pd.to_numeric(details_one_gene.loc[sampling_ixs_select_model].s0), pd.to_numeric(details_one_gene.loc[sampling_ixs_select_model].u0),
pd.to_numeric(details_one_gene.loc[sampling_ixs_select_model].s1)-pd.to_numeric(details_one_gene.loc[sampling_ixs_select_model].s0), pd.to_numeric(details_one_gene.loc[sampling_ixs_select_model].u1)-pd.to_numeric(details_one_gene.loc[sampling_ixs_select_model].u0)
)
plt.show()

detail = pd.read_csv('/Users/shengyuli/OneDrive - Houston Methodist/work/Velocity/data/velocyto/neuro/detailcsv/denGyrLr0.001Costv1C1r0.5C2cf0.3Downneighbors0_200_200Ratio0.125N30OAdam/detail_e200.csv')
####################################
# color_map="coolwarm"
color_scatter=None
alpha_inside=1
