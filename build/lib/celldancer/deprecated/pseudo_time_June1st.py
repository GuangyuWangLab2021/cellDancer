...
def compute_trajectory_similarity(traj1, traj2, numPicks=10):
    ...
    return np.mean(np.sqrt(A))


def compute_trajectory_similarity_terminal_state(traj1, traj2, numPicks=10):
    '''
    Computes the similarity between two curves based on whether the shorter
    trajectory traj2 ends close to traj1
    ...
    temp = traj1 - traj2[-1, None]
    A = np.min(np.sum(temp**2, axis=1))
    return np.sqrt(A)
...
def extract_representative_long_trajectories(
    ...
    longest = paths[0]
    # NNN new modification
    similarity = np.array([
        compute_trajectory_similarity_terminal_state(np.array(longest), np.array(ipath), 10) 
...
def recur_cell_time_assignment_intracluster(
    ...
    ZERO_TIME = min(unresolved_cell_time_cluster.values())
    DURATION = TERMINAL_TIME - ZERO_TIME
    ZERO_TIME = ZERO_TIME + 0.*DURATION
    TERMINAL_TIME = TERMINAL_TIME - 0.*DURATION

    #print(unresolved_cell_time_cluster)
    ...
    zero_time_cells = [cellid for cellid, celltime in 
            unresolved_cell_time_cluster.items() if celltime <= ZERO_TIME]
...
def cell_time_assignment_intercluster(
    ...
    for cluster_index in range(len(clusterIDs)):
        ...
        node_sizes[clusterIDs[cluster_index]] = len(time_cluster)
    nx.set_node_attributes(CT, node_sizes, "cluster size")
    ...
    paths = nx.get_edge_attributes(CT,'weight')
    nx.draw_networkx_edge_labels(CT,pos,edge_labels=paths)
    plt.show()
    ...
    for tree_nodes in nx.weakly_connected_components(CT):
        ...
        ct_sub = nx.subgraph(CT, tree_nodes)
        temp_w_cumm = relative_time_in_tree(ct_sub)
...
def relative_time_in_tree(G):
    paths = nx.get_edge_attributes(G, 'weight')
    nodes = nx.get_node_attributes(G, 'cluster size')

    def cycle_breaker():
        regCells = {path:nodes[path[0]] + nodes[path[1]] for path in paths}

        while not nx.is_tree(G):
            print("There exists a cycle in the cluster graph.")
            print("Removing edges ...")
            ignored_edge = sorted_nodes.pop(0)
            G.remove_node(ignored_node)
            G.add_node(ignored_node)
        return G
    ...
    while 0 in flag.values():
        ...
        for path in paths:
            if path[0]==node:
                ...
                w_cumm[path[1]] = w_cumm[node]+paths[path]
                node = path[1]
                flag[node] = 1

            elif path[1]==node:
                #print("Backward: "+str(node)+" -> "+str(path[0]))
                w_cumm[path[0]] = w_cumm[node]-paths[path]
                node=path[0]
                flag[node] = 1
            else:
                #print("Pass: "+str(node))
...
def interpolate_all_cell_time(cell_time, all_cell_embedding, sampling_ixs,
    ...
    all_cell_time[all_cell_time>np.quantile(all_cell_time, 0.95)] = \
            np.quantile(all_cell_time, 0.95)
...
def compute_cell_time(
    ...
    cell_time_per_cluster = [
            projection_cell_time_one_cluster(
                cell_embedding,
                path_clusters[cluster][0], 
                cluster, 
                cell_fate) for cluster in clusters]
...
def pseudo_time(
    ...
    def binary_search(s_high, s_low, n_path_min, n_path_max):
        if n_path_min > n_paths:
            print("minimum number of paths: ", n_path_min)
            return s_high
        if n_path_max < n_paths:
            print("maximum number of paths: ", n_path_max)
            return s_low
...
def plot_celltime_clusters(cell_time_per_cluster, path_clusters, embedding):
    ...
    for i in range(n_paths):
        ...
        velocity = lead_path[1:,:] - lead_path[:-1,:]
        velocity = np.linalg.norm(velocity, axis=-1)
        
        plt.scatter(cells[:,0], cells[:,1], 
                c=cell_time, s=5, cmap = colormap, alpha=0.3)
        #plt.scatter(lead_path[:,0], lead_path[:,1], 
        #        c=range(len(lead_path)), s=5, cmap = 'Reds_r')
        plt.scatter(lead_path[1:,0], lead_path[1:,1], 
                c=range(len(lead_path)-1), s=10000*velocity, cmap = 'Reds',
                alpha=0.5)
