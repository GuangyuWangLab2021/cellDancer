...
def recur_cell_time_assignment_intracluster(
    ...
    if isinstance(MAX_TERMINAL_CELLS, float):
        MAX_TERMINAL_CELLS = int(MAX_TERMINAL_CELLS \
                * len(unresolved_cell_time_cluster))
    MAX_TERMINAL_CELLS = max(MAX_TERMINAL_CELLS, 10)

    print("Total cells in the cluster: ", len(unresolved_cell_time_cluster))
    print("MAX allowed zero time cells: ", MAX_ZERO_TIME_CELLS)
    print("MAX allowed terminal cells: ", MAX_TERMINAL_CELLS)
    ...
    if len(zero_time_cells) < MAX_ZERO_TIME_CELLS:
        ...
        # add the longest trajectory belonging to the zero-time cells to refpaths.
    ...
    # For terminal cells
    if len(terminal_cells) < MAX_TERMINAL_CELLS:
        ...
        # add the longest trajectory belonging to the terminal cells to refpaths.
        ref_path = sorted_traj[0]
        # re-assign time for the terminal cells
        sub_cell_time = cell_time_projection_cluster(
                cell_embedding, 
                ref_path, 
                cluster, 
    ...
    if len(zero_time_cells+terminal_cells) == 0:
        return unresolved_cell_time_cluster, \
...
def compute_all_cell_time(
    ...
    for clusterID in clusters:
        refPaths= recur_cell_time_assignment_intracluster(
        ...
        #print("Cell time for subclusters: ")
        ...
        if len(cell_time_subclusters) > 1:
            ...
            #print("Subclusters for cluster ", clusterID)
            #print(cell_fate_subclusters_dict)
...
def plot_celltime_clusters(cell_time_per_cluster, embedding):
    ...
    n_paths = len(cell_time_per_cluster)
