...
def simulation(
    ...
    def simulate_dynamics(tau, alpha, beta, gamma, u0, s0, noise_model, noise_level):
        ...
        spliced = (
            s0 * exps + alpha / gamma * (1 - exps) + c * (exps - expu)
        )
        ut = unspliced
        st = spliced
        print(ut)
...
adata_0=simulation(n_obs=300, n_vars=1, alpha=1, beta=0.1, gamma=0.21, alpha_=None, t_max=None, noise_model='normal', noise_level=0.1, switches=None, random_seed=0)
plt.figure()
plt.scatter(adata_0.layers['unspliced'],adata_0.layers['spliced'])


adata_0=simulation(n_obs=3, n_vars=1, alpha=10, beta=0.1, gamma=0.21, alpha_=None, t_max=None, noise_model='normal', noise_level=0.1, switches=None, random_seed=0)
plt.figure()
plt.scatter(adata_0.layers['unspliced'],adata_0.layers['spliced'])