#
# change file name to velocity estimistation/celldancer
    ...
    def cost_fn(self, u0, s0, alpha0, beta0, gamma0, barcode = None, dt = 0.5):
        ...
        def trace_cost(u0, s0, u1, s1, idx,version):
            ...
            if version=="v1":
                ...
                one_tensor=torch.tensor(np.full((idx.size()[0]), 1))
        ...
        cost1=(cost1-torch.min(cost1))/torch.max(cost1)
        cost2=(cost2-torch.min(cost2))/torch.max(cost2)
        # print("---cost1------")
        ...
        if cost2_mean<0.3:           
            ...
            # cost_mean = 0.5*cost1_mean + 0.5*cost2_mean


        return cost_mean, u1, s1, alphas, beta, gamma # to do
...
class realDataMododule(pl.LightningDataModule): #change name to feedData
    '''
...
def training_realdata_figure1b():
    ...
    from sampling import sampling_adata
    ...
    # process of den_gyr data
    import pandas as pd
    den_gyr=pd.read_csv("velocyto/data/denGyr.csv",names=['gene_list', 'u0','s0','clusters'])
    ...
    gene_choice=["Ank","Abcc8","Tcp11","Nfib","Ppp3ca",
            ...
            "Nnat","Anxa4","Actn4","Btbd17","Dcdc2a",
            "Adk","Smoc1","Mapre3","Pim2","Tspan7",
            "Top2a","Rap1b","Sulf2","Ntrk2","Tmem163"]
    ...
    # plt.scatter(data_df[data_df.gene_list=="Tmem163"].s0,data_df[data_df.gene_list=="Tmem163"].u0)
    # plt.scatter(data_df_downsampled[data_df_downsampled.gene_list=="Tmem163"].s0,data_df_downsampled[data_df_downsampled.gene_list=="Tmem163"].u0,c="red")
    ...
    datamodule = realDataMododule(data_fit = data_df_downsampled, data_predict=data_df, sampling_ratio=0.125)
    ...
    detail_e1000.to_csv("output/detailcsv/adj_e/detail_e1000.csv")
    ...
    brief_e200.to_csv("output/detailcsv/adj_e/brief_e200.csv")
    brief_e300.to_csv("output/detailcsv/adj_e/brief_e300.csv")
    brief_e400.to_csv("output/detailcsv/adj_e/brief_e400.csv")
    brief_e500.to_csv("output/detailcsv/adj_e/brief_e500.csv")
    brief_e1000.to_csv("output/detailcsv/adj_e/brief_e1000.csv")
